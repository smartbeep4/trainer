...
![Self-attention visualized: A heatmap showing attention weights between tokens in a short sequence. Darker cells indicate stronger relevance between a query token and key tokens.](/images/course/attention-heatmap.jpg)

*This diagram illustrates how self-attention computes dynamic, content-based relevance scores across all pairs of tokens in parallel.*
...
![The internal structure of a single Transformer block, showing the multi-head attention sub-layer with residual connection and layer normalization, followed by the position-wise feed-forward network with its own residual connection and normalization.](/images/course/transformer-block.jpg)

*A single transformer layer (block) combines attention, feed-forward processing, residuals, and normalization. Dozens or hundreds of these are stacked.*
